...
struct RangeAnalysisResult
{
    ...
    string tacticalDirection;           // "BUY_AT_SUPPORT", "SELL_AT_RESISTANCE", "WAIT_IN_MIDDLE"
    string positionContext;             // "PRICE_NEAR_SUPPORT", "PRICE_NEAR_RESISTANCE", "PRICE_IN_MIDDLE", "UNKNOWN"
    string touchDominance;              // "SUPPORT_DOMINANT", "RESISTANCE_DOMINANT", "BALANCED", "NO_TOUCHES", "UNKNOWN"
    double touchRatio;                  // supportTouches / resistanceTouches (0.0 if no touches)
    string strongestEdge;               // "SUPPORT", "RESISTANCE", "BALANCED", "UNKNOWN"
    double distanceToSupportPercent;    // How far from support as % of range
    double distanceToResistancePercent; // How far from resistance as % of range
    ...
    // Constructor - INITIALIZE ALL NEW FIELDS
    RangeAnalysisResult()
    {
        ...
        // Initialize NEW context fields
        tacticalDirection = "WAIT_IN_MIDDLE";
        positionContext = "UNKNOWN";
        touchDominance = "UNKNOWN";
        touchRatio = 1.0;
        strongestEdge = "UNKNOWN";
        distanceToSupportPercent = 0;
        distanceToResistancePercent = 0;
    }

    // Simple output - ENHANCED WITH CONTEXT
    string ToString() const
    {
        if (!isValidRange)
        {
        ...
        return StringFormat(
            "Overall: %s (%.0f%%) | Range: %.1f%% width | Trap: %.0f%% | Bias: %s (%.0f%%) | Action: %s | Context: %s | Touch: %s (%.2f:1)",
            overallDirection, overallConfidence, rangeWidthPercent, trapProbability,
            rangeBiasDirection, rangeBiasConfidence, rangeAction,
            positionContext, touchDominance, touchRatio);
    }

    // Detailed output - ENHANCED WITH CONTEXT
    string ToDetailedString() const
    {
        ...
        details += "\n=== CONTEXT ANALYSIS ===\n";
        details += StringFormat("Tactical Direction: %s\n", tacticalDirection);
        details += StringFormat("Position Context: %s\n", positionContext);
        details += StringFormat("Touch Dominance: %s (Ratio: %.2f:1)\n", touchDominance, touchRatio);
        details += StringFormat("Strongest Edge: %s\n", strongestEdge);
        details += StringFormat("Distance to Support: %.1f%% | Distance to Resistance: %.1f%%\n",
                                distanceToSupportPercent, distanceToResistancePercent);
    ...
    {
        if (componentName == "Tightness")
            return scores.tightnessAnalysis;
        if (componentName == "Symmetry")
            return scores.symmetryAnalysis;
        if (componentName == "Consolidation Time")
            return scores.consolidationTimeAnalysis;
        if (componentName == "Touch Quality")
            return scores.touchQualityAnalysis;
        if (componentName == "Rejection")
            return scores.rejectionAnalysis;
        if (componentName == "Failed Breakouts")
            return scores.failedBreakoutAnalysis;
        if (componentName == "Liquidity")
            return scores.liquidityAnalysis;
        if (componentName == "HTF Alignment")
            return scores.higherTimeframeAnalysis;
        if (componentName == "Round Numbers")
            return scores.roundNumberAnalysis;
        if (componentName == "Session")
            return scores.sessionAnalysis;
...
namespace RangeIntelligence
{
    ...
    //+------------------------------------------------------------------+
    //| Calculate component directions and strengths (ENHANCED)          |
    //+------------------------------------------------------------------+
                                    ...
                                    RangeAnalysisResult &result)
    {
        ...
        // 1. TIGHTNESS ANALYSIS (unchanged)
        ...
        result.scores.tightnessAnalysis.confidence = 80;
        if (rangeWidthPercent < VERY_TIGHT_RANGE)
        {
            result.scores.tightnessAnalysis.direction = "BEARISH";
        ...
        // ============ ENHANCED SYMMETRY ANALYSIS WITH CONTEXT ============
        ...
        // Calculate touch ratio for context
        double touchRatio = 1.0;
        if (touchesResistance > 0)
        {
            touchRatio = (double)touchesSupport / touchesResistance;
        }
        result.touchRatio = touchRatio; // Store in result

        // Determine touch dominance
        if (touchesSupport == 0 && touchesResistance == 0)
        {
            result.touchDominance = "NO_TOUCHES";
            result.scores.symmetryAnalysis.direction = "NEUTRAL";
            result.scores.symmetryAnalysis.reasoning = "No touches detected on either side";
        }
        else if (touchRatio > 1.8)
        {
            result.touchDominance = "SUPPORT_DOMINANT";
            result.scores.symmetryAnalysis.direction = "BULLISH";
            result.scores.symmetryAnalysis.reasoning = StringFormat("Strong support dominance (%d:%d = %.1f:1) - buyers defending",
                                                                    touchesSupport, touchesResistance, touchRatio);
        }
        else if (touchRatio > 1.3)
        {
            result.touchDominance = "SUPPORT_LEANING";
            result.scores.symmetryAnalysis.direction = "BULLISH";
            result.scores.symmetryAnalysis.reasoning = StringFormat("Moderate support leaning (%d:%d = %.1f:1)",
                                                                    touchesSupport, touchesResistance, touchRatio);
        }
        else if (touchRatio < 0.56)
        { // 1/1.8 ≈ 0.56
            result.touchDominance = "RESISTANCE_DOMINANT";
            result.scores.symmetryAnalysis.direction = "BEARISH";
            result.scores.symmetryAnalysis.reasoning = StringFormat("Strong resistance dominance (%d:%d = 1:%.1f) - sellers defending",
                                                                    touchesSupport, touchesResistance, 1.0 / touchRatio);
        }
        else if (touchRatio < 0.77)
        { // 1/1.3 ≈ 0.77
            result.touchDominance = "RESISTANCE_LEANING";
            result.scores.symmetryAnalysis.direction = "BEARISH";
            result.scores.symmetryAnalysis.reasoning = StringFormat("Moderate resistance leaning (%d:%d = 1:%.1f)",
                                                                    touchesSupport, touchesResistance, 1.0 / touchRatio);
        }
        else
        {
            result.touchDominance = "BALANCED";
            result.scores.symmetryAnalysis.direction = "NEUTRAL";
            result.scores.symmetryAnalysis.reasoning = StringFormat("Balanced touches (%d support, %d resistance)",
                                                                    touchesSupport, touchesResistance);
        }

        // ============ CALCULATE PRICE POSITION CONTEXT ============
        double rangeHeight = resistance - support;
        if (rangeHeight > 0)
        {
            result.distanceToSupportPercent = ((currentPrice - support) / rangeHeight) * 100;
            result.distanceToResistancePercent = ((resistance - currentPrice) / rangeHeight) * 100;

            // Determine position context
            if (result.distanceToSupportPercent < 15)
            {
                result.positionContext = "PRICE_NEAR_SUPPORT";
            }
            else if (result.distanceToResistancePercent < 15)
            {
                result.positionContext = "PRICE_NEAR_RESISTANCE";
            }
            else if (result.distanceToSupportPercent > 35 && result.distanceToResistancePercent > 35)
            {
                result.positionContext = "PRICE_IN_MIDDLE";
            }
            else
            {
                result.positionContext = "PRICE_IN_RANGE";
            }
        }

        // 3. CONSOLIDATION TIME ANALYSIS (unchanged)
        ...
        {
            result.scores.consolidationTimeAnalysis.direction = "BEARISH";
        ...
        // 4. TOUCH QUALITY ANALYSIS (unchanged)
        ...
        // 5. REJECTION STRENGTH ANALYSIS (unchanged)
        ...
        // 6. FAILED BREAKOUT ANALYSIS (unchanged)
        ...
        // 7. LIQUIDITY ANALYSIS (unchanged)
        ...
        // 8. HIGHER TIMEFRAME ALIGNMENT (unchanged)
        ...
        // 9. ROUND NUMBER ANALYSIS (unchanged)
        ...
        // ============ SET STRONGEST EDGE BASED ON MULTIPLE FACTORS ============
        // Consider touch quality, rejection strength, and touch dominance
        double supportScore = (result.scores.touchQualityScore * 0.3) +
                              (CountStrongRejections(symbol, timeframe, support, true) * 20.0) +
                              ((result.touchDominance == "SUPPORT_DOMINANT" || result.touchDominance == "SUPPORT_LEANING") ? 30.0 : 0.0);

        double resistanceScore = (result.scores.touchQualityScore * 0.3) +
                                 (CountStrongRejections(symbol, timeframe, resistance, false) * 20.0) +
                                 ((result.touchDominance == "RESISTANCE_DOMINANT" || result.touchDominance == "RESISTANCE_LEANING") ? 30.0 : 0.0);

        if (supportScore > resistanceScore * 1.2)
        {
            result.strongestEdge = "SUPPORT";
        }
        else if (resistanceScore > supportScore * 1.2)
        {
            result.strongestEdge = "RESISTANCE";
        }
        else
        {
            result.strongestEdge = "BALANCED";
        }
    ...
    //+------------------------------------------------------------------+
    //| Calculate range-based directional bias with context             |
    //+------------------------------------------------------------------+
                            ...
                            ENUM_TIMEFRAMES timeframe, double currentPrice)
    {
        if (!result.isValidRange)
        {
            ...
            result.tacticalDirection = "WAIT_IN_MIDDLE";
            return;
        }
        ...
        double rangeHeight = result.resistanceLevel - result.supportLevel;

        // ============ DETERMINE PRICE POSITION CONTEXT ============
        result.positionContext = "UNKNOWN";
        if (rangeHeight > 0)
        {
            double supportDistancePercent = (distanceToSupport / rangeHeight) * 100;
            double resistanceDistancePercent = (distanceToResistance / rangeHeight) * 100;

            if (supportDistancePercent < 15)
            {
                result.positionContext = "PRICE_NEAR_SUPPORT";
            }
            else if (resistanceDistancePercent < 15)
            {
                result.positionContext = "PRICE_NEAR_RESISTANCE";
            }
            else if (supportDistancePercent > 35 && resistanceDistancePercent > 35)
            {
                result.positionContext = "PRICE_IN_MIDDLE";
            }
            else
        ...
        if (nearSupport && distanceToSupport < distanceToResistance)
        {
            ...
            result.tacticalDirection = "BUY_AT_SUPPORT";
            result.positionContext = "PRICE_NEAR_SUPPORT";
            ...
            // ============ CONTEXT ENHANCEMENT: TOUCH DOMINANCE ============
            if (result.touchDominance == "SUPPORT_DOMINANT")
            {
                // Strong case: Price at support + support-dominant touches
                result.rangeBiasConfidence *= 1.25; // 25% confidence boost
                result.rangeBiasReason = StringFormat(
                    "🎯 STRONG BUY: Price at support (%.5f) with SUPPORT-DOMINANT touches (%d:%d = %.1f:1) - expecting strong bounce",
                    result.supportLevel, result.supportTouches, result.resistanceTouches, result.touchRatio);
                result.rangeAction = "FADE_EDGE_STRONG";
            }
            else if (result.touchDominance == "SUPPORT_LEANING")
            {
                // Moderate case: Price at support + support-leaning touches
                result.rangeBiasConfidence *= 1.1; // 10% confidence boost
                result.rangeBiasReason = StringFormat(
                    "✅ BUY: Price at support (%.5f) with support-leaning touches (%d:%d = %.1f:1) - expecting bounce",
                    result.supportLevel, result.supportTouches, result.resistanceTouches, result.touchRatio);
                result.rangeAction = "FADE_EDGE";
            }
            else if (result.touchDominance == "RESISTANCE_DOMINANT" || result.touchDominance == "RESISTANCE_LEANING")
            {
                // Warning: Price at support but resistance-dominant touches
                result.rangeBiasConfidence *= 0.7; // 30% confidence reduction
                result.rangeBiasReason = StringFormat(
                    "⚠️ CAUTIOUS BUY: Price at support (%.5f) but RESISTANCE-DOMINANT touches (%d:%d = 1:%.1f) - weak bounce expected",
                    result.supportLevel, result.supportTouches, result.resistanceTouches, 1.0 / result.touchRatio);
                result.rangeAction = "FADE_EDGE_WEAK";
            }
            else if (result.touchDominance == "BALANCED")
            {
                // Neutral case: Price at support with balanced touches
                result.rangeBiasReason = StringFormat(
                    "📊 BUY: Price at support (%.5f) with balanced touches (%d:%d) - neutral bounce expectation",
                    result.supportLevel, result.supportTouches, result.resistanceTouches);
                result.rangeAction = "FADE_EDGE";
            }
            else
            {
        ...
        else if (nearResistance && distanceToResistance < distanceToSupport)
        {
            ...
            result.tacticalDirection = "SELL_AT_RESISTANCE";
            result.positionContext = "PRICE_NEAR_RESISTANCE";
            ...
            // ============ CONTEXT ENHANCEMENT: TOUCH DOMINANCE ============
            if (result.touchDominance == "RESISTANCE_DOMINANT")
            {
                // Strong case: Price at resistance + resistance-dominant touches
                result.rangeBiasConfidence *= 1.25; // 25% confidence boost
                result.rangeBiasReason = StringFormat(
                    "🎯 STRONG SELL: Price at resistance (%.5f) with RESISTANCE-DOMINANT touches (%d:%d = 1:%.1f) - expecting strong rejection",
                    result.resistanceLevel, result.supportTouches, result.resistanceTouches, 1.0 / result.touchRatio);
                result.rangeAction = "FADE_EDGE_STRONG";
            }
            else if (result.touchDominance == "RESISTANCE_LEANING")
            {
                // Moderate case: Price at resistance + resistance-leaning touches
                result.rangeBiasConfidence *= 1.1; // 10% confidence boost
                result.rangeBiasReason = StringFormat(
                    "✅ SELL: Price at resistance (%.5f) with resistance-leaning touches (%d:%d = 1:%.1f) - expecting rejection",
                    result.resistanceLevel, result.supportTouches, result.resistanceTouches, 1.0 / result.touchRatio);
                result.rangeAction = "FADE_EDGE";
            }
            else if (result.touchDominance == "SUPPORT_DOMINANT" || result.touchDominance == "SUPPORT_LEANING")
            {
                // Warning: Price at resistance but support-dominant touches
                result.rangeBiasConfidence *= 0.7; // 30% confidence reduction
                result.rangeBiasReason = StringFormat(
                    "⚠️ CAUTIOUS SELL: Price at resistance (%.5f) but SUPPORT-DOMINANT touches (%d:%d = %.1f:1) - weak rejection expected",
                    result.resistanceLevel, result.supportTouches, result.resistanceTouches, result.touchRatio);
                result.rangeAction = "FADE_EDGE_WEAK";
            }
            else if (result.touchDominance == "BALANCED")
            {
                // Neutral case: Price at resistance with balanced touches
                result.rangeBiasReason = StringFormat(
                    "📊 SELL: Price at resistance (%.5f) with balanced touches (%d:%d) - neutral rejection expectation",
                    result.resistanceLevel, result.supportTouches, result.resistanceTouches);
                result.rangeAction = "FADE_EDGE";
            }
            else
            {
        ...
        else
        {
            result.rangeBiasDirection = "NEUTRAL";
            result.tacticalDirection = "WAIT_IN_MIDDLE";
            result.positionContext = "EQUIDISTANT";
            result.rangeBiasReason = "Equidistant from both range edges";
            result.rangeBiasConfidence = 0;
            result.rangeAction = "WAIT_IN_MIDDLE";
        }

        // ============ HIGH TRAP PROBABILITY ADJUSTMENT ============
        if (result.trapProbability > 85)
        {
            ...
            result.rangeBiasConfidence *= 0.5; // Halve confidence for extreme traps
            result.tacticalDirection = "AVOID_TRADE";
        }
        else if (result.trapProbability > 70)
        {
            result.rangeBiasConfidence *= 0.8; // Reduce confidence for high traps
            result.rangeBiasReason += " | High trap probability - be cautious";
        }
                                   ...
                                   double support, double resistance)
    {
        ...
        if (avgRangeAtTouch > avgATR * 1.5)
            return 80;
        else if (avgRangeAtTouch > avgATR)
            return 60;
        else if (avgRangeAtTouch > avgATR * 0.7)
            return 40;
        else
            return 20;
    }
                                 ...
                                 double support, double resistance)
    {
        ENUM_TIMEFRAMES higherTF = GetHigherTimeframe(currentTF);
        if (higherTF == currentTF)
            return 50; // No higher TF available
                                    ...
                                    double atr, string biasDirection)
    {
        ...
        if (rangeWidth > atr * 10)
            score *= 1.2;
        else if (rangeWidth > atr * 5)
            score *= 1.1;
    ...
    double GetTrapRiskMultiplier(string symbol, ENUM_TIMEFRAMES tf = PERIOD_H1)
    {
        ...
        if (result.trapProbability > 85)
            ...
            return 0.7;
        else if (result.trapProbability > 50)
            return 0.8;
        else
            return 1.0;
    }
