...
public:
   ...
   MTFAnalyser(IndicatorManager *manager = NULL)
   {
      ...
      m_initialized = false;
      m_lastDisplayTime = 0;
      m_use89EMAFilter = true;

      // Initialize timeframe array structure
      ...
      m_timeframeWeights[6] = 2.0; // D1

      // ✅ Set indicator manager if provided, otherwise it will be set in Initialize()
      m_indicatorManager = manager;

      DebugLogMTF("Constructor", "MTFAnalyser created");
   }
   ...
   bool Initialize(string symbol = NULL, ENUM_TIMEFRAMES primaryTF = PERIOD_CURRENT,
                   IndicatorManager *indicatorManager = NULL) // Changed to pointer
   {
      ...
      // Set timeframe
      m_primaryTF = (primaryTF == PERIOD_CURRENT) ? Period() : primaryTF;
      DebugLogMTF("Initialize", "Primary TF: " + IntegerToString(m_primaryTF));

      // ============================================================
      // ✅✅✅ UPDATED: USE SINGLETON INSTEAD OF CREATING NEW INSTANCES
      // ============================================================
      if (indicatorManager != NULL)
      {
         // Use provided pointer (which should be the singleton)
         m_indicatorManager = indicatorManager;
         DebugLogMTF("Initialize", "Using provided IndicatorManager (should be singleton)");
      }
      else
      {
         // ✅ GET THE SINGLETON INSTANCE - THIS IS THE KEY CHANGE!
         m_indicatorManager = IndicatorManager::Instance();
         DebugLogMTF("Initialize", "✅ Using IndicatorManager singleton");

         if (m_indicatorManager == NULL)
         {
            DebugLogMTF("Initialize", "❌ ERROR: Failed to get IndicatorManager singleton");
            return false;
         }
      }
      ...
      if (m_indicatorManager == NULL)
      {
         DebugLogMTF("Initialize", "❌ ERROR: IndicatorManager is NULL");
         return false;
      }

      if (CheckPointer(m_indicatorManager) == POINTER_INVALID)
      {
         DebugLogMTF("Initialize", "❌ ERROR: IndicatorManager pointer is invalid");
         return false;
      }

      // Verify IndicatorManager is initialized
      if (!m_indicatorManager.IsInitialized())
      {
         DebugLogMTF("Initialize", "⚠️ IndicatorManager singleton not initialized, initializing now...");
         if (!m_indicatorManager.Initialize())
         {
            DebugLogMTF("Initialize", "❌ Failed to initialize IndicatorManager singleton");
            return false;
         }
      }

      DebugLogMTF("Initialize", "✅ IndicatorManager singleton is valid and initialized");

      // Test indicator access
      DebugLogMTF("Initialize", "Testing indicator access...");
      double testMA1, testMA2, testMA3;
      if (!m_indicatorManager.GetMAValues(m_primaryTF, testMA1, testMA2, testMA3))
      {
         DebugLogMTF("Initialize", "❌ Failed to get MA values for testing");
         return false;
      }

      DebugLogMTF("Initialize", StringFormat("✅ MA test successful: %.4f, %.4f, %.4f",
                                             testMA1, testMA2, testMA3));
...
private:
   ...
   TrendAnalysis AnalyzeTrendWithEMA89(string symbol, ENUM_TIMEFRAMES timeframe)
   {
      ...
      DebugLogMTF("AnalyzeTrendWithEMA89", "Basic trend: " +
                                               (basicTrend == TREND_UP ? "UP" : basicTrend == TREND_DOWN   ? "DOWN"
                                                                            : basicTrend == TREND_SIDEWAYS ? "SIDEWAYS"
                                                                                                           : "UNCLEAR"));
      ...
      DebugLogMTF("AnalyzeTrendWithEMA89", "Final analysis: Trend=" +
                                               (analysis.trend == TREND_UP ? "UP" : analysis.trend == TREND_DOWN   ? "DOWN"
                                                                                : analysis.trend == TREND_SIDEWAYS ? "SIDEWAYS"
                                                                                                                   : "UNCLEAR") +
   ...
   {
      if (score.bullishWeightedScore > score.bearishWeightedScore)
         return "BULLISH";
      if (score.bearishWeightedScore > score.bullishWeightedScore)
         return "BEARISH";
      return "NEUTRAL";
   }
