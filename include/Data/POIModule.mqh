...
struct POIModuleSignal
{
    ...
    {
        if (overallBias == POI_BIAS_BULLISH)
            return "BUY";
        if (overallBias == POI_BIAS_BEARISH)
            return "SELL";
        return "HOLD";
    }
    ...
    string GetDisplayString() const
    {
        ...
        if (overallBias == POI_BIAS_BULLISH)
            biasIcon = "▲";
        else if (overallBias == POI_BIAS_BEARISH)
            biasIcon = "▼";
...
struct POIScores
{
    ...
    string GetDominantDirection() const
    {
        if (bullScore > bearScore && bullScore > 50)
            return "BULLISH";
        if (bearScore > bullScore && bearScore > 50)
            return "BEARISH";
        return "NEUTRAL";
    }
};
...
public:
    ...
    void SetDisplayMode(int mode)
    {
        ...
        {
            m_displayMode = mode;
            if (m_initialized && m_drawOnChart)
                DrawZonesOnChart();
        }
    }
    ...
    {
        if (!m_initialized || count <= 0 || count > m_zoneCount)
            return false;
    ...
    {
        if (!m_initialized)
            return "HOLD - POI Not Initialized";
    ...
    string GetOverallBias(double currentPrice)
    {
        ...
        if (GetNearestZone(currentPrice, nearestZone))
        {
            ...
            else
            {
                ...
                {
                    if (nearestZone.type == POI_SUPPORT)
                        return "BULLISH";
                    else
                        return "BEARISH";
                }
    ...
    void OnTick()
    {
        ...
        if (m_drawOnChart)
        {
            ...
            static datetime lastTickDraw = 0;
            tickCounter++;

            // Only draw every 500 ticks AND at least 30 seconds between draws
            if (tickCounter >= 500 && (TimeCurrent() - lastTickDraw >= 30))
            {
                DrawZonesOnChart();
                tickCounter = 0;
                lastTickDraw = TimeCurrent();
            }
        }
    }
...
private:
    ...
    {
        if (index < lookback || index >= ArraySize(data) - lookback)
            return false;
    ...
    void DrawZonesOnChart()
    {
        ...
        // Only redraw if:
        // 1. More than 30 seconds have passed, OR
        // 2. Price moved significantly (> 0.5 ATR), OR
        // 3. It's been more than 5 minutes since last draw

        if (TimeCurrent() - lastDrawTime < 30 &&
            MathAbs(currentPrice - lastPrice) < (m_currentATR * 0.5) &&
            TimeCurrent() - lastDrawTime < 300)
        {
            return;
        }
        ...
        POIZone sortedZones[];
        if (!GetNearestZones(sortedZones, m_maxDisplayZones, currentPrice))
            return;
    ...
    void DrawZone(const POIZone &zone, int rank)
    {
        ...
        // Check if object already exists with same properties
        bool needsUpdate = true;
        if (ObjectFind(m_chartId, baseName) >= 0)
        {
            double existingPrice = ObjectGetDouble(m_chartId, baseName, OBJPROP_PRICE);
            color existingColor = (color)ObjectGetInteger(m_chartId, baseName, OBJPROP_COLOR);
            color expectedColor = (zone.type == POI_SUPPORT) ? m_supportColor : m_resistanceColor;

            // Only update if price changed significantly or color changed
            if (MathAbs(existingPrice - zone.priceLevel) < (m_currentATR * 0.01) &&
                existingColor == expectedColor)
            {
                needsUpdate = false;
            }
        }
        ...
        if (!needsUpdate)
        {
            // Update the label only if zone exists (keeps label updated without blinking)
    ...
    void DrawSimpleZones()
    {
        ...
        double currentPrice = SymbolInfoDouble(m_symbol, SYMBOL_BID);
        POIZone zones[];
        if (!GetNearestZones(zones, m_maxDisplayZones, currentPrice))
            return;
    ...
    string FormatDistance(double distance)
    {
        ...
        if (pips < 10)
            return StringFormat("%.1f", pips);
        else if (pips < 1000)
            return StringFormat("%.0f", pips);
        else
            return StringFormat("%.1fK", pips / 1000);
    }
    ...
    ENUM_POI_BIAS StringToBias(string biasString)
    {
        if (biasString == "BULLISH")
            return POI_BIAS_BULLISH;
        if (biasString == "BEARISH")
            return POI_BIAS_BEARISH;
        if (biasString == "CONFLICTED")
            return POI_BIAS_CONFLICTED;
    ...
    double CalculateSignalConfidence(POIModuleSignal &signal, const POIZone &zone)
    {
        ...
        if (m_indicatorManager != NULL)
        {
            ...
            double rsi = m_indicatorManager.GetRSI(PERIOD_M15, 0);
    ...
    double CalculateDistanceFactor(double distance, double buffer)
    {
        ...
        if (distance <= buffer * 2)
            return 1.0 - ((distance - buffer) / (buffer * 2));
    ...
    string DetermineSignalPriceAction(const POIModuleSignal &signal, double currentPrice, const POIZone &zone)
    {
        ...
        if (m_indicatorManager != NULL)
        {
            bool trendBullish = m_indicatorManager.IsTrendBullish(PERIOD_M15);
            bool trendBearish = m_indicatorManager.IsTrendBearish(PERIOD_M15);
    ...
    {
        if (signal.overallBias == POI_BIAS_NEUTRAL)
            return false;
    ...
    {
        if (TimeCurrent() - m_lastCleanupTime < 3600)
            return;
